...
const load = async () => {
  ...
  rows.forEach((m) => {
      c[m.id as string] = (m.ministerio_membros as {count: number}[])?.[0]?.count ?? 0;
      ac[m.id as string] = (m.areas as {count: number}[])?.[0]?.count ?? 0;
  });
  setCounts(c);
  setAreaCounts(ac);
...
const aplicarRefatoracao = async (
  ...
  const usuarioEmail = (user as { email?: string } | null)?.email ?? null;

  // Salva historico antes
  ...
  decisoes.forEach((decisao, item) => {
      if (decisao === "atualizar") {
            promises.push(
                    aplicarItemRefatoracao(item, "atualizar", usuarioEmail).catch(() => { erros++; })
                  );
      }
      if (decisao === "criar") {
            promises.push(
                    aplicarItemRefatoracao(item, "criar", usuarioEmail).catch(() => { erros++; })
                  );
      }
  });
  ...
  if (erros > 0) {
      ...
      if (acoes.length > 0) {
            toast.success("Refatoracao concluida: " + acoes.join(", "));
      } else {
            toast.info("Nenhuma alteracao aplicada.");
      }
  }
...
const aplicarModelo = () => {
  ...
  setForm((f) => ({ ...f, descricao: partes.join("\n\n") }));
  setSugestao(null);
  toast.success(
  ...
  <div>
    <PageHeader
                              ...
                              <Card key={m.id} className={`shadow-card-soft ${m.ativo ? "" : "opacity-60"}`}>
                                          <CardContent className="p-5">
                                                        <div className="flex items-start gap-3">
                                                                        <div className="w-10 h-10 rounded-md bg-primary/10 flex items-center justify-center"><HeartHandshake className="w-5 h-5 text-primary"/></div>div>
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
          <DialogContent>
                <DialogHeader><DialogTitle className="font-serif text-2xl">{editingId ? "Editar ministerio" : "Novo ministerio"}</DialogTitle>DialogTitle></DialogHeader>DialogHeader>
                <form onSubmit={onSubmit} className="space-y-3">
                  ...
                  {sugestao && (
                      <div className={`rounded-md border px-3 py-2.5 flex items-start gap-2 ${
                                    sugestao.origem === "documento" ? "border-gold/50 bg-gold/8" : "border-muted bg-muted/30"
                      }`}>
                                  ...
                                  <div className="flex-1 min-w-0">
                                                <p className={`text-xs font-medium ${sugestao.origem === "documento" ? "text-gold" : "text-foreground"}`}>
                                                  {sugestao.origem === "documento"
                                                                      ...
                                                                      : "Modelo padrao: " + sugestao.nome}
                                                </p>p>
                                    {sugestao.base_institucional && (
                                        <p className="text-[10px] text-gold/70 mt-0.5">{sugestao.base_institucional}</p>p>
                                                )}
      ...
      {/* Modal de Refatoracao Inteligente */}
