...
export default function Dashboard() {
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      setCasamentos(
        (cas ?? [])
          .filter((p: any) => inWeek(p.data_casamento))
          .map((p: any) => ({ ...p, data_nascimento: p.data_casamento }))
          .slice(0, 8),
      );
      setLoaded(true);
    } catch (e: any) {
  ...
  const cards = [
    ...
    { label: "Ativos", sub: "em comunhão", value: stats.ativos ?? 0, icon: Heart },
    { label: "Famílias", sub: "núcleos familiares", value: stats.familias ?? 0, icon: Home as any },
    { label: "Ministérios", sub: "cadastrados", value: stats.ministerios ?? 0, icon: HeartHandshake },
  ];
  ...
  return (
    <div>
      ...
      <div className="p-4 md:p-8 space-y-6">
        {/* Versículo do dia — destaque institucional */}
        <Card className="overflow-hidden border-0 shadow-elevated bg-gradient-verse text-foreground relative">
          ...
          <CardContent className="p-8 md:p-10 relative">
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="text-[11px] tracking-[0.2em] uppercase text-gold/90 mb-3">Versículo do dia</div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {cards.map((c) => (
            <Card
              key={c.label}
              translate="no"
              className="shadow-card-soft border border-border/60 hover:shadow-elevated transition-shadow"
            >
              <CardContent className="p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <p translate="no" className="text-xs tracking-wider uppercase text-muted-foreground">
                      {c.label}
                    </p>
                    {loaded ? (
                    ...
                    <p translate="no" className="text-xs text-muted-foreground mt-1">
                      {c.sub || "—"}
                    </p>
                  </div>
                  ...
                  {aniversariantes.map((p) => (
                    <li key={p.id} className="py-2.5 flex justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {format(parse(p.data_nascimento, "yyyy-MM-dd", new Date()), "dd 'de' MMMM", { locale: ptBR })}
                      </span>
                    </li>
                  ))}
                </ul>
          ...
          <Card className="shadow-card-soft">
            ...
            <CardContent>
              {!loaded ? (
                ...
                <p className="text-sm text-muted-foreground">Nenhum aniversário de casamento nesta semana.</p>
              ) : (
                <ul className="divide-y">
                  {casamentos.map((p) => (
                    <li key={p.id} className="py-2.5 flex justify-between text-sm">
